    ...
    try:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                chat_message = ChatMessage(ChatRoom=chat, Sender=user, Message=UserMessage)
                chat_message.save()
            ...
            try:
                ...
                if user.user_type == "Buyer":
                    ...
                    for chatData in chatsData:
                        responseData.append({"fullName":chatData.Seller.full_name, "profilePicture": chatData.Seller.profile_pic ,"email":chatData.Seller.email_address,"lastMessage":chatData.LastMessage, "time": chatData.modified.strftime("%H:%M %p"), "chatID": chatData.id})
                else:
                    chatsData = Chat.objects.filter(Seller=user)
                    for chatData in chatsData:
                        responseData.append({"fullName":chatData.Buyer.full_name, "profilePicture": chatData.Seller.profile_pic ,"email":chatData.Buyer.email_address,"lastMessage":chatData.LastMessage , "time": chatData.modified.strftime("%H:%M %p"), "chatID": chatsData.id})
